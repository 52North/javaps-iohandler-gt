 ...
 *
 */
public class GeoJSONParserGeneratorTest extends AbstractTestCase {

    @Inject
    private GeoJSONParser theParser;
    @Inject
    private GeoJSONGenerator dataHandler;
    ...
    public void testParseWriteGeoJSONPoint() {
        ...
        JTSGeometryBinding theBinding = null;
        try {
            theBinding = (JTSGeometryBinding) theParser.parse(null, in, format);
        ...
        try {
            generatedStream = dataHandler.generate(null, theBinding, format);
    ...
    public void testParseWriteGeoJSONFeatureCollection(){
        ...
        Data<?> theBinding = null;
        try {
            theBinding = theParser.parse(null, in, format);
        } catch (IOException | DecodingException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
        }
        ...
        try {
            InputStream is = dataHandler.generate(null, theBinding, format);

            Data<?> theGeneratedParsedBinding = theParser.parse(null, is, format);
